...
class payload {
    ...
    public:
        ...
        SET(setOldParent, int, OldParent, _Old);
        GET(getOldParent, int, OldParent);
...
class IoT_device: public node {
        ...
        map<unsigned int, unsigned int> reverse_p;
...
class IoT_sink: public node {
        ...
        bool hi; // this is used for example; you can remove it when doing hw2
        unsigned int parent;
        map<unsigned int, set<unsigned int>> ACG_msn;
        map<unsigned int, unsigned int> reverse_p;
        map<unsigned int, unsigned int> n_parent;
...
void IoT_device::recv_handler (packet *p){
    ...
    if (p->type() == "IoT_ctrl_packet") { // the device receives a packet from the sink
        ...
        if(p3->getHeader()->getSrcID() == getNodeID()){
            l3->setOldParent(parent);
            const map<unsigned int, bool> &nblist = getPhyNeighbors();
            ostringstream neighbor_str;
            for (map<unsigned int, bool>::const_iterator it = nblist.begin(); it != nblist.end(); it++){
                if(it->second){
                    neighbor_str << it->first << " ";
                }
            }
            string result = neighbor_str.str();
            if (!result.empty()) {
                result.pop_back();
            }
            //cout << result << endl;
            ...
            reverse_p[p3->getHeader()->getSrcID()] = p3->getHeader()->getPreID();
        }
        ...
        if(p3->getHeader()->getDstID() == getNodeID()){
            ...
            p3->getHeader()->setPreID(getNodeID());
            p3->getHeader()->setNexID(reverse_p[p3->getHeader()->getDstID()]);
            send_handler(p3);
        }
...
void IoT_sink::recv_handler (packet *p){
    ...
    else if (p->type() == "AGG_ctrl_packet")
    {
        ...
        n_parent[p3->getHeader()->getSrcID()] = l3->getOldParent();
        reverse_p[p3->getHeader()->getSrcID()] = p3->getHeader()->getPreID();

        istringstream iss(l3->getMsg());
        set<unsigned int> ready;
        ...
        cout << endl;*/
        ACG_msn[p3->getHeader()->getSrcID()] = ready;
        // cout << "node id = " << getNodeID() << ", msg = "  << l3->getMsg() << endl;
    }
    else if (p->type() == "DIS_ctrl_packet")
    {
        ...
        for(int i = 1; n_parent.find(i) != n_parent.end(); i++){
            l3->setParent(n_parent[i]);
            p3->getHeader()->setPreID(getNodeID());
            p3->getHeader()->setNexID(reverse_p[i]);
...
int main(){
    ...
    int Nodes, Links, SimTime, BS_Time, Data_Trans_Time, Packet_Size, ACG_Start, DIS_Start;
    cin >> Nodes >> Links >> Packet_Size;
    cin >> SimTime >> BS_Time >> ACG_Start;
    cin >> DIS_Start >> Data_Trans_Time;
    ...
    for(int i = 0; i < Nodes; i++){
        cin >> Current >> Data_Size;
    }

    int Now_ID, Now_End1, Now_End2;
    // please generate the sink by yourself
    // set devices' neighbors
    for(int i = 0; i < Links; i++){
        cin >> Now_ID >> Now_End1 >> Now_End2;
    ...
    for(int node_id = 1; node_id < Nodes; node_id++){
        AGG_ctrl_packet_event(node_id, 0, ACG_Start);
    }
    // 1st parameter: the source node
    // 2nd parameter: the destination node (sink)
    // 3rd parameter: time (optional)
    ...
    DIS_ctrl_packet_event(0, DIS_Start);
